...
import { PhysicalNoteScannerSettings } from '../types';
import { FILE_PREFIX } from '../constants';

export class ImageProcessor {
	constructor(
		private app: App,
		private settings: PhysicalNoteScannerSettings
	) {}
	 ...
	 */
	async saveImage(imageData: ArrayBuffer, originalFilename: string): Promise<TFile> {
	...
	private async getUploadFolder(): Promise<string> {
		if (this.settings.uploadFolder && this.settings.uploadFolder.trim() !== '') {
			// Use custom folder
	 ...
	 */
	private async getAvailablePath(filename: string, folder: string): Promise<string> {
