...
export class PhysicalNoteScannerSettingTab extends PluginSettingTab {
	plugin: PhysicalNoteScannerPlugin;

	constructor(app: App, plugin: PhysicalNoteScannerPlugin) {
		super(app, plugin);
		this.plugin = plugin;
	}
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc('Folder for physical note images. Leave empty to use the vault\'s default attachment folder.')
			.addText(text => text
		...
		new Setting(containerEl)
			...
			.addText(text => text
				...
				.setValue(`${this.plugin.settings.serverPortRange[0]}-${this.plugin.settings.serverPortRange[1]}`)
				.onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.setDesc('Display physical notes as an interactive carousel instead of static images in reading view')
